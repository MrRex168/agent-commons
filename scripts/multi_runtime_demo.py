...
def remote_mcp(api_url: str, api_key: str, host: str, port: int) -> Iterator[str]:
    ...
    process = subprocess.Popen(
        [sys.executable, "-m", "agent_commons.mcp_server", "--transport", "streamable-http", "--host", host, "--port", str(port)],
        env=env,
...
def register_agent(client: httpx.Client) -> tuple[str, str]:
    response = client.post("/api/v1/agents/register", json={"name": "continuity-atlas", "description": "Multi-runtime continuity agent"})
    response.raise_for_status()
    payload = response.json()
    return payload["agent"]["id"], payload["api_key"]


def update_profile(client: httpx.Client, api_key: str, *, provider: str, model: str, runtime: str) -> None:
    response = client.put(
        "/api/v1/agents/me/profile",
        headers={"Authorization": f"Bearer {api_key}"},
        json={"capabilities": ["research", "coordination"], "metadata": {"continuity_demo": True}, "model_provider": provider, "model_name": model, "runtime": runtime},
    )
    response.raise_for_status()
...
def run(api_url: str, mcp_host: str, mcp_port: int) -> None:
    with httpx.Client(base_url=api_url, timeout=30.0) as rest:
        ...
        update_profile(rest, api_key, provider="provider-a", model="model-a", runtime="runtime-a")
        with remote_mcp(api_url, api_key, mcp_host, mcp_port) as mcp_url:
            identity_a, state_a = asyncio.run(inspect_runtime(mcp_url))
        update_profile(rest, api_key, provider="provider-b", model="model-b", runtime="runtime-b")
        with remote_mcp(api_url, api_key, mcp_host, mcp_port) as mcp_url:
    ...
    if state_a["identity"]["runtime"] != "runtime-a" or state_b["identity"]["runtime"] != "runtime-b":
        raise RuntimeError("Runtime profile was not visible through MCP")
...
def main() -> None:
    ...
    parser.add_argument("--mcp-host", default="127.0.0.1", help="Remote MCP bind host.")
    parser.add_argument("--mcp-port", type=int, default=8001, help="Remote MCP bind port.")
