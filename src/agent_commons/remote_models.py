...
class RemoteAgentReference(Base):
    ...
    card_url: Mapped[str] = mapped_column(String(500), unique=True, index=True, nullable=False)
    ...
    root_fingerprint: Mapped[str | None] = mapped_column(String(80), unique=True, index=True)
    current_controller_public_key: Mapped[str | None] = mapped_column(String(128), nullable=True)
